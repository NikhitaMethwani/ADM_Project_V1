...
import pandas as pd
# from custom_book_suggestion import custom_book_search
...
def api_reco():
    # begin 04/20
    # button_val = flask.request.form["sub"]
    ...
    if button_val == "Submit":
        if flask.request.form["custom_title"] != "":
            # custom_title = flask.request.form["custom_title"]
    ...
    first_line = True
    # book, user = read_book_details(custom_title, title, uid)
    book, user = read_book_details(title, uid)
    # book, user = read_book_details(gen, custom_title, lan, title, uid)
    # book, user = read_book_details(title, uid)
